...
- **印刷頁碼判讀**：每頁回傳 `[HEADER LINES]` / `[FOOTER LINES]`，由 LLM 自行判讀頁碼（支援 `- 92 -`、`2-13`、`IX` 等各種格式）
...
- **Session 記憶**：所有 `read_pdf_pages` / `search_pdf_text` 結果自動存入 `output/session_reads.md`，micro_compact 後仍可取回
- **Context 壓縮**：自動 micro-compact + auto-compact，支援長對話；壓縮前後印出 token 數
...
│   └── session_reads.md  # 本次對話所有 PDF 讀取記錄（自動產生）
├── skills/
...
7. `read_pdf_pages` 逐頁確認，每頁回傳 `[HEADER LINES]` / `[FOOTER LINES]`；LLM 判讀印刷頁碼
...
| `read_pdf_pages` | 讀取文字層（TOC 導航、頁碼確認用），附 HEADER/FOOTER LINES |
| `search_pdf_text` | 全文關鍵字搜尋，回傳 physical page + footer 內容 |
...
不同文件的 footer 格式差異很大，無法用單一 regex 通用解析：

| 文件類型 | Footer 範例 | 頁碼 |
|---------|------------|------|
| MRT 手冊 | `SYL-TK01-OPM-ESN-0005-0A - 92 - OCT, 2025` | 92 |
| 政府報告 | `2-13` | 2-13（章節頁碼） |
| 前言 | `IX` | 羅馬數字 |

因此改由 LLM 從 `[HEADER LINES]` / `[FOOTER LINES]` 自行判讀，能跨文件格式正確識別。

### Session 記憶（session_reads.md）

每次呼叫 `read_pdf_pages` 或 `search_pdf_text`，結果都會自動 append 到 `output/session_reads.md`。

micro_compact 壓縮舊 tool result 時，CONTENT_TOOLS（PDF 讀取類）的 content 替換為：
```
[Compacted — full content saved in output/session_reads.md]
```

LLM 看到此訊息時，可呼叫 `read_file("output/session_reads.md")` 取回完整內容。

### Context 壓縮策略

| 層次 | 觸發條件 | 動作 |
|------|----------|------|
| micro_compact | 超過 6 個舊 tool result | 清除舊 tool 結果（保留 load_skill）；PDF 讀取結果存入 session_reads.md |
| auto_compact | 估計 token > 50,000 | 摘要整段對話，存 transcript |
| manual compact | 呼叫 `compact` 工具 | 同 auto_compact |

壓縮前後會印出 token 數：
```
[context: 12,345 → 8,210 tokens after micro_compact]
```
