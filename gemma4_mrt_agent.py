...
def agent_loop(messages: list):
    ...
    while True:
        ...
        try:
            ...
            print(f"[LLM: {time.time() - _t_llm:.1f}s]")
        except Exception as e:
        ...
        for block in msg.tool_calls:
            ...
            if name == "task":
                ...
                try:
                    ...
                    output = f"Error: {e}"
            _elapsed = time.time() - _t_tool

            if name == "todo":
                print(f"> todo [{_elapsed:.2f}s]:\n{output}")
                used_todo = True
            elif name == "list_pdfs":
                print(f"> list_pdfs [{_elapsed:.2f}s]:\n{str(output)}")
            elif name == "read_pdf_pages":
                preview = str(output)[:1500]
                print(f"> read_pdf_pages (p.{args.get('pages', '?')}) [{_elapsed:.2f}s]:\n{preview}...")
            elif name == "search_pdf_text":
                q = args.get("query", "")
                preview = str(output)[:800]
                print(f"> search_pdf_text ({q!r}) [{_elapsed:.2f}s]:\n{preview}...")
            elif name == "render_pdf_pages":
                print(f"> render_pdf_pages (p.{args.get('pages', '?')}) [{_elapsed:.2f}s]: {str(output)[:300]}")
            elif name == "open_files":
                print(f"> open_files [{_elapsed:.2f}s]: {str(output)[:200]}")
            elif name == "write_note":
                print(f"> write_note (note_id={args.get('note_id','?')}) [{_elapsed:.2f}s]: {str(output)[:200]}")
            elif name == "read_file":
                print(f"> read_file ({args.get('path','?')}) [{_elapsed:.2f}s]: {str(output)[:200]}")
            elif name == "web_search":
                print(f"> web_search ({args.get('query', '')!r}) [{_elapsed:.2f}s]: {str(output)[:200]}...")
            else:
                print(f"> {name} [{_elapsed:.2f}s]: {str(output)[:200]}")
